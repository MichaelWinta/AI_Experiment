...
public class KI3 {
    ...
    private static boolean hasWon(char[][] map, int posX, int posY) {
        int top = KI3.getConnectedCoins(map, posX, posY, 0, -1);
        int bottom = KI3.getConnectedCoins(map, posX, posY, 0, 1);
        int vertical = top + bottom + 1;

        int left = KI3.getConnectedCoins(map, posX, posY, -1, 0);
        int right = KI3.getConnectedCoins(map, posX, posY, 1, 0);
        int horizontal = left + right + 1;

        int leftTop = KI3.getConnectedCoins(map, posX, posY, -1, -1);
        int rightBottom = KI3.getConnectedCoins(map, posX, posY, 1, 1);
        int diagonalBottom = leftTop + rightBottom + 1;

        int leftBottom = KI3.getConnectedCoins(map, posX, posY, -1, 1);
        int rightTop = KI3.getConnectedCoins(map, posX, posY, 1, -1);
        ...
        if (isPlayer) {
            ...
            if (KI3.hasWon(map, posX, posY)) {
                ...
                for (int y = 0; y < map.length && run; y++) {
                    for (int x = 0; x < map[0].length && run; x++) {
                        if (map[y][x] == '*') {
                            double newScore = KI3.scoreMove(map, x, y, meToken, opponentToken, false, maxDepth-1, alpha, beta);
            ...
            if (KI3.hasWon(map, posX, posY)) {
                ...
                for (int y = 0; y < map.length && run; y++) {
                    for (int x = 0; x < map[0].length && run; x++) {
                        if (map[y][x] == '*') {
                            double newScore = KI3.scoreMove(map, x, y, meToken, opponentToken, true, maxDepth-1, alpha, beta);
    ...
    public static String move(final char[][] map, char meToken, char opponentToken) {
        ...
        if (moveCount >= 4) {
            ...
            for (int y = 0; y < map.length; y++) {
                for (int x = 0; x < map[0].length; x++) {
                    if (map[y][x] == '*') {
                        double newScore = KI3.scoreMove(newMap, x, y, meToken, opponentToken, true, 7, -2.0, beta);
                        // System.out.println(String.valueOf(x) + "x" + String.valueOf(y) + ": " + String.valueOf(newScore));
                        double newDistance = KI3.getDistance(x, y);
                        if (newScore > bestScore || (newScore == bestScore && newDistance < bestDistance)) {
                            bestScore = newScore;
                            moveX = x;
                            moveY = y;
